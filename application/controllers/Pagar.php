...
class Pagar extends CI_Controller {
    ...
    public function del($conta_pagar_id = null){
        if(!$conta_pagar_id || !$this->core_model->get_by_id('contas_pagar',array('conta_pagar_id'=> $conta_pagar_id))){
            ...
            $this->core_model->delete('contas_pagar', array('conta_pagar_id' => $conta_pagar_id,'conta_pagar_status'=>1));
            $this->session->set_flashdata('info', 'Esta conta não pode ser paga, pois ainda esta em aberto');
            redirect('pagar');
        }
        $this->core_model->delete('contas pagar', array('conta pagar_id' => $conta_pagar_id)); 
        redirect('pagar');
    }
